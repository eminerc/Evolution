    ...
    def move(self, cells, food, surface, surface_width, surface_height):
        def find_scope(x):
            if self.y_cell != self.y_goal and self.x_cell != self.x_goal:
                ...
                try:
                    z = 10 / math.sqrt(1 + (((self.y_goal - self.y_cell) / (self.x_goal - self.x_cell)) * (
                                (self.y_goal - self.y_cell) / (self.x_goal - self.x_cell))))
                except:
                    z = 10 / 0.000000000001
                ...
                z1 = k / math.sqrt(1 + (-1 * ((self.x_goal - self.x_cell) / (self.y_goal - self.y_cell))) * (
                            -1 * ((self.x_goal - self.x_cell) / (self.y_goal - self.y_cell))))
                ...
                m2 = (self.y_cell - ((self.y_cell + z * (
                            (self.y_goal - self.y_cell) / (self.x_goal - self.x_cell))) + mxy * z1)) / (
                                 self.x_cell - (self.x_cell + z + z1))
                z2 = (self.p_radius / 2 + (self.s_radius * 20 / 510)) / math.sqrt(1 + m2 * m2)

                if (self.x_goal - self.x_cell) == 0:
                    m3 = (self.y_cell - (
                                (self.y_cell + z * ((self.y_goal - self.y_cell) / 0.000000001)) - mxy * z1)) / (
                                     self.x_cell - (self.x_cell + z - z1))
                elif (self.x_cell - (self.x_cell + z - z1)) == 0:
                    m3 = (self.y_cell - ((self.y_cell + z * (
                                (self.y_goal - self.y_cell) / (self.x_goal - self.x_cell))) - mxy * z1)) / 0.00000000001
                else:
                    m3 = (self.y_cell - ((self.y_cell + z * (
                                (self.y_goal - self.y_cell) / (self.x_goal - self.x_cell))) - mxy * z1)) / (
                                     self.x_cell - (self.x_cell + z - z1))
                ...
                points1 = [[self.x_cell - z5, self.y_cell - (z5 * m5)], [self.x_cell - z4, self.y_cell - (z4 * m4)],
                           [self.x_cell + z5, self.y_cell + (z5 * m5)], [self.x_cell + z4, self.y_cell + (z4 * m4)]]

                points = [[self.x_cell + z2, self.y_cell + (z2 * m2), 0],
                          [self.x_cell + z3, self.y_cell + (z3 * m3), 0],
                          [self.x_cell - z2, self.y_cell - (z2 * m2), 0],
                          [self.x_cell - z3, self.y_cell - (z3 * m3), 0]]
                ...
                if math.sqrt((x[0] - self.x_cell) ** 2 + (x[1] - self.y_cell) ** 2) < self.p_radius / 2 + (
                        self.s_radius * 20 / 255):
                    if (points[0][0] > self.x_cell and points[1][0] > self.x_cell) or (
                            points[0][0] < self.x_cell and points[1][0] < self.x_cell):
                        if self.y_goal > self.y_cell:
                            if abs(((self.y_cell - points[1][1]) / (self.x_cell - points[1][0]))) < abs(
                                    ((self.y_cell - points[0][1]) / (self.x_cell - points[0][0]))):
                                if x[1] - self.y_cell > ((self.y_cell - points[1][1]) / (self.x_cell - points[1][0])) * (
                                        x[0] - self.x_cell) and x[1] - self.y_cell < (
                                        (self.y_cell - points[0][1]) / (self.x_cell - points[0][0])) * (x[0] - self.x_cell):
                                    return True
                                else:
                                    return False
                            else:
        ...
        def find_goal():
            ...
            if len(dc) > 0 and dc[0][0] < 20 + (self.s_radius * 20 / 510) and dc[0][1].s_radius >= self.s_radius:
                ...
                for i in self.memory[:]:
                    #print([i[1].x_food, i[1].y_food])
                    if find_scope([i[0], i[1]]) and math.sqrt((i[0] - self.x_cell) ** 2 + (i[1] - self.y_cell) ** 2) <= self.p_radius/2 + (
                            self.s_radius * 20 / 510):
                        self.memory.remove(i)
                for i in df:
                    #print([i[1].x_food, i[1].y_food])
                    if find_scope([i[1].x_food, i[1].y_food]) and i[0] <= self.p_radius/2 + (self.s_radius * 20 / 510):
                        self.memory.append([i[1].x_food, i[1].y_food, 0])
                df = sorted(df, key=lambda k: k[0])
                for i in food:
                    df.append([math.sqrt((i.x_food - self.x_cell) ** 2 + (i.y_food - self.y_cell) ** 2), i])
                df = sorted(df, key=lambda k: k[0])
                if len(df) > 0:
